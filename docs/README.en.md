...
├── perf/                 — performance methodology + analysis
├── plugins/              — per-plugin reference pages (livedoc-generated)
├── img/                  — SVG diagrams (regenerated by
│                           tools/gen_diagrams.py)
├── _facts/               — livedoc-generated YAML facts files
│                           (host_api slots, link/handler/security
│                           vtables, config keys, metrics, RFC
│                           coverage, roadmap status, test inventory)
...
├── dx-sdk-sugar.en.md    — modern SDK sugar (`connect_to`, `listen_to`,
│                           `Subscription`) for plugin authors
