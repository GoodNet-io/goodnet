...
│   ├── rust/             — Rust authors (TBD)
│   └── python/           — Python authors (TBD)
├── img/                  — SVG diagrams (regenerated by
│                           tools/gen_diagrams.py)
├── architecture.canvas   — bird's-eye summary poster
├── ROADMAP.md            — shipped and planned features
├── threat-model.md       — adversary positions and enforcement
├── install.md            — production deployment
├── LICENSES.md           — per-component licence matrix
└── Doxyfile              — API-reference generation
```

`docs/impl/rust/` and `docs/impl/python/` are placeholders for
language guides that follow the C++ template once those bindings
land. Top-level introduction and quickstart material — «как этим
пользоваться» from a first-time operator's view — lives outside
this developer corpus and grows in its own pass.
