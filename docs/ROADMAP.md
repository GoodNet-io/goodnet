...
| IPC transport plugin | shipped — Boost.Asio `local::stream_protocol` with strand-per-session writes (mirrors TCP shape per `transport.md` §4); listen `chmod 0700` on the parent directory before bind to close the TR-C6 TOCTOU window, `unlink` only when the existing path is a socket; declares `GN_TRUST_LOOPBACK`; URI form `ipc:///path/to/sock` per `uri.md` is_path_style; ASan + TSan clean |
| UDP transport plugin | shipped — Boost.Asio `udp::socket` with single strand for both send and recv (datagram transports cannot serialise with per-session strands), MTU-gated send path with all-or-nothing batch precheck, per-source `RateLimiterMap` on new-conn allocation, `notify_disconnect` on every released peer; URI form `udp://host:port` per `uri.md`, `connect`-side rejects port 0 per §5; ASan + TSan + clang-tidy strict clean |
