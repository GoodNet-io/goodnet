...
| TCP transport plugin | shipped — Asio with strand-per-session writes (`transport.md` §4 single-writer), idempotent `shutdown_.exchange(true)`, IPv6 wildcard `IPV6_V6ONLY=false` for dual-stack listens; OBJECT lib for in-tree tests + `goodnet_link_tcp.so` plugin entry |
