...
Например «rich peer-discovery system» = handler-plugin
(mDNS scanner) плюс extension `gn.peer-info` плюс app
`goodnet-explore`.
...
2. Bare mirror однократно: `git clone --bare <plugin-checkout>
   ~/.local/share/goodnet-mirrors/<kind>-<name>.git`. Plugin's
   `origin` указывает на mirror.
3. Внутри plugin checkout итерация: правка `<plugin>.cpp`,
   `nix run .#test`, `nix run .#test-tsan`, коммит на `dev`.
   Kernel suite от этого ничего не пересобирает — фаза идёт
   изолированно.
4. Готовый блок едет в `feat/<topic>` ветку, мерж `--no-ff` в
   `dev`, push на mirror.
5. В kernel checkout `plugins/CMakeLists.txt` уже знает про
   слот; `nix run .#test` от kernel root запускает kernel + все
...
App checkout настроен симметрично plugin checkout'у:

- `flake.nix` имеет goodnet input на kernel-only subflake — та
  же модель что и plugin
- `CMakeLists.txt`:
  ```cmake
...
Binding checkout содержит:

- `flake.nix` с goodnet input на kernel-only subflake
- `CMakeLists.txt` с INTERFACE target
- Header'ы или языковой модуль, который wraps capi
- `tests/test_compile.cpp` (или языковой эквивалент) — smoke
  test что каждый header компилируется standalone
- `nix run .#test` запускает compile-test изолированно

Kernel root подбирает binding автоматически:
...
`GoodNet::cpp` target живёт когда binding checkout'ан, и app'ы
которые его требуют (`gssh`) собираются автоматически. Без
binding checkout'а apps skip'аются — их CMakeLists guard'ятся
через `TARGET GoodNet::cpp`.
