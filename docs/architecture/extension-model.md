...
- Контракт: [host-api.md](../contracts/host-api.md) — slot'ы `register_extension` / `query_extension_checked`, error semantics, lifetime.
- Контракт: [conn-events.md](../contracts/conn-events.md) — pub/sub для асинхронной координации между плагинами.
- Контракт: [security-trust.md](../contracts/security-trust.md) — TrustClass и upgrade, на который плагины часто реагируют через query.
- Архитектура: [overview](./overview.md) — место расширений в общей картине.
- Архитектура: [plugin-model](./plugin-model.md) — четыре роли плагина и их vtable shapes.
- Архитектура: [host-api-model](./host-api-model.md) — KIND-tagged primitives, ABI evolution.
- Архитектура: [relay-direct](./relay-direct.md) — конкретный пример plugin↔plugin координации через расширения.
