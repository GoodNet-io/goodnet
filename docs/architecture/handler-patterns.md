...
[`plugin-model`](./plugin-model.md) описывает 4 vtable kind'а на
уровне «handler — один из них». [`routing`](./routing.md) показывает
дorgan dispatcher chain. [`handler-registration.md`](../contracts/handler-registration.md) —
...
2. **Snapshot dispatch** — kernel снимает priority-desc snapshot
   ...
   [`handler-registration.md §3`](../contracts/handler-registration.md).
...
[`extension-model`](./extension-model.md), kernel agnostic — только
registry.
...
- `0x12` — capability TLV (per [`capability-tlv.md`](../contracts/capability-tlv.md))
- `0x11` — attestation dispatch (kernel-managed,
  [`attestation.md`](../contracts/attestation.md))
...
- [`recipes/subscribe-conn-events.md`](../recipes/subscribe-conn-events.md) —
  cleanup handler-side state on DISCONNECTED
- [`recipes/register-extension.md`](../recipes/register-extension.md) —
  handler-as-publisher pattern (handler + extension surface)
- [`recipes/instrument-with-metrics.md`](../recipes/instrument-with-metrics.md) —
  emit_counter через api в handler'е
- [`routing`](./routing.md) — kernel router внутрь handler chain
- [`extension-model`](./extension-model.md) — как handler-as-publisher
  resolved'ится через `query_extension_checked`
- [`overview`](./overview.md) §«Что знает ядро» — kernel agnostic
  про handlers, registry-only
- [`capability-tlv.md`](../contracts/capability-tlv.md) — wire
  format для capability negotiation msg ids
- [`limits.md`](../contracts/limits.md) — `max_handlers_per_msg_id`
  и другие quota
