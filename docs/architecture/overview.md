...
взаимодействуют с ним через тот же контракт `protocol-layer.md`,
что и любой внешний layer.
...
См. [plugin-model](./plugin-model.md) и
[extension-model](./extension-model.md) — две peer-страницы этого
...
[host-api](../contracts/host-api.md)). Структура начинается с
...
Подробнее — на странице [host-api-model](./host-api-model.md).
...
`abi-evolution.md`. Архитектурный слой — этот — рассказывает, как
...
[kernel-fsm](./kernel-fsm.md).

---

## С чего начать

**Автору плагина.** Прочесть [host-api](../contracts/host-api.md) и
[plugin-lifetime](../contracts/plugin-lifetime.md), затем контракт
для своей роли — `link.md`, `handler-registration.md`,
`security-trust.md`. Дальше — `impl/<язык>/` со шаблонами под
конкретный FFI-биндинг.

**Контрибьютору ядра.** Войти через [kernel-fsm](./kernel-fsm.md) и
[fsm-events](../contracts/fsm-events.md), затем
[plugin-model](./plugin-model.md) и
[host-api-model](./host-api-model.md). Регистры разобраны на
`docs/contracts/registry.md` и в исходниках под `core/registry/`.

**Оператору, отлаживающему развёртывание.** Начать с
[connection-lifecycle](./connection-lifecycle.md) и
[backpressure-model](./backpressure-model.md), потом
[metrics](../contracts/metrics.md). FSM-фазы и сигналы — это первый
