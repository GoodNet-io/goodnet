...
GoodNet — специализированное ядро для сетевого слоя. Центральная предпосылка: **адрес есть ключ**. 32-байтовый Ed25519 публичный ключ — это пир, а не машина; не локация в топологии, а криптографически верифицируемая идентичность. Ядро держит соединение в состоянии `Untrusted` до тех пор, пока mutual-attestation не даёт `Peer` — и этот переход не обходится ни конфигурацией, ни опцией. CA не нужен. DNS не нужен для доверия. `known_hosts` не существует — ключ и есть идентичность.

Из этой предпосылки вытекает форма ядра. Каждое решение о транспорте, шифровании, маршрутизации, протоколах приложений — снаружи, в плагинах за стабильным C ABI. Ядро держит реестры и диспетчерский цикл. Ядро без плагинов ничего не делает; набор плагинов оператора — это не конфигурация, а определение того, чем система является.

Тот же бинарник работает как Linux-демон, как `.so` через FFI, как WebAssembly-модуль в браузере и как bare-metal на микроконтроллере. Это не результат портирования — ядро никогда не предполагало конкретного окружения. Link-плагин владеет I/O; ядро не знает что такое сокет.
...
**Плагины** (тринадцать подгружаемых единиц) живут каждый в своём
репозитории. Handler-плагины `heartbeat`, `store`, `dns`;
link-плагины `tcp`, `udp`, `ws`, `ipc`, `tls`, `ice`, `quic`;
security-плагины `noise`, `null`; strategy-плагин `float_send_rtt`.
К ним примыкают sibling-репозитории `integration-tests` и
`bridges-cpp`. Каждый плагин собирается как `.so`, грузится через
`dlopen` после проверки SHA-256-манифеста и регистрирует свои
vtable через `host_api`.

**Apps** — операторская сторона. Бинарь `goodnetd`, демо,
инструменты тестирования. Все они держат `gn_core_t*` через `sdk/core.h`,
...
_1441 test cases across 20 tree(s)._

| Tree | Cases | Files |
|---|---:|---:|
| [`tests/`](../../tests) | 972 | 90 |
| [`plugins/handlers/dns`](../../plugins/handlers/dns) | 73 | 4 |
| [`plugins/handlers/heartbeat`](../../plugins/handlers/heartbeat) | 11 | 1 |
| [`plugins/handlers/store`](../../plugins/handlers/store) | 51 | 2 |
| [`plugins/handlers/web_api_proxy`](../../plugins/handlers/web_api_proxy) | 7 | 1 |
| [`plugins/links/ice`](../../plugins/links/ice) | 44 | 2 |
| [`plugins/links/ipc`](../../plugins/links/ipc) | 8 | 2 |
| [`plugins/links/portmap`](../../plugins/links/portmap) | 26 | 2 |
...
| [`plugins/links/ws`](../../plugins/links/ws) | 15 | 2 |
| [`plugins/links/ws_inject`](../../plugins/links/ws_inject) | 9 | 1 |
| [`plugins/protocols/gnet`](../../plugins/protocols/gnet) | 63 | 4 |
| [`plugins/security/noise`](../../plugins/security/noise) | 56 | 5 |
| [`plugins/security/null`](../../plugins/security/null) | 18 | 1 |
| [`plugins/security/pkcs11`](../../plugins/security/pkcs11) | 8 | 3 |
| [`plugins/strategies/float_send_rtt`](../../plugins/strategies/float_send_rtt) | 10 | 1 |
| [`plugins/workers/remote_echo`](../../plugins/workers/remote_echo) | 3 | 1 |
<!-- /livedoc:test_inventory_table -->
