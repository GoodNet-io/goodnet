...
Симметричная сторона. Если плагин решил «вернуться на relay» — он зовёт `disconnect(direct_conn)` и `dial("relay://R/<pk>")`, ровно тем же шаблоном последовательного переключения, что и upgrade в обратную сторону. Ничего нового; см. [multi-path](./multi-path.md) §«Шаблон последовательного переключения».
...
- Архитектура: [multi-path](./multi-path.md) — общий шаблон последовательного переключения путей.
- Архитектура: [extension-model](./extension-model.md) — публикация и query расширений `gn.peer-info` / `gn.heartbeat`.
- Архитектура: [security-flow](./security-flow.md) — handshake, attestation, переход trust class в `Peer`.
- Контракт: [conn-events.md](../contracts/conn-events.md) — `TRUST_UPGRADED` / `DISCONNECTED` event payload, ordering, threading.
- Контракт: [host-api.md](../contracts/host-api.md) — slot'ы `subscribe_conn_state`, `query_extension_checked`, `disconnect`.
- Контракт: [security-trust.md](../contracts/security-trust.md) — one-way upgrade `Untrusted → Peer`, attestation-gate.
- Контракт: [link.md](../contracts/link.md) — `connect(self, uri)` slot, `notify_connect` от link'а.
- Контракт: [uri.md](../contracts/uri.md) — grammar для URI-схем, включая `relay://` и гипотетический `ice://`.
