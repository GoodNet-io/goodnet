...
1. Захватить shard-mutex для `id`, URI-index-mutex и pk-index-mutex
   ...
   локи идентично.
2. Проверить, что `conn_id` свободен в shard map; иначе — отпустить
   локи и вернуть `GN_ERR_LIMIT_REACHED`. URI и pk-индексы
   *перезаписывают* существующую entry — это primitive multi-conn
   (несколько соединений к одному peer'у через один или разные URI).
   Lookup-by-URI и lookup-by-pk возвращают conn'у, *последней
   записавшей* индекс; полный набор живых conn'ов виден через
   `for_each_connection`.
