...
запрос «выбери conn из этого набора». Контракт описан в
`sdk/extensions/strategy.h`:

```c
gn_result_t (*pick_conn)(void* ctx,
                          const uint8_t peer_pk[GN_PUBLIC_KEY_BYTES],
                          const gn_path_sample_t* candidates,
                          size_t candidate_count,
                          gn_conn_id_t* out_chosen);

gn_result_t (*on_path_event)(void* ctx,
                              const uint8_t peer_pk[GN_PUBLIC_KEY_BYTES],
                              gn_path_event_t ev,
                              const gn_path_sample_t* sample);
```

Что делит `gn.strategy.*` (kernel-mediated, kernel = router) от
`gn.float-send.*` (plugin-owned-send, plugin = router):

| Family            | Send pipe       | Conn map ownership | Use case |
|-------------------|-----------------|--------------------|----------|
...
Первый strategy plugin: `plugins/strategies/float_send_rtt/`.
Класс `FloatSendRtt`:

- Per-conn smoothed RTT (EWMA, α = 1/8 per RFC 6298) обновляется
  на `GN_PATH_EVENT_RTT_UPDATE`.
- `pick_conn` выбирает min-RTT кандидата.
- Hysteresis: ⩾ 25 % улучшение требуется для flip с current winner
  (operator-tunable `set_switch_threshold`).
- Tie-band (±5 %): `EncryptedPath`-capable conn выигрывает у
  plain'ого при близких RTT.
- `CONN_DOWN` evict'ит conn из tracking + winner если совпадает.
