...
| 2026-06-15 | `gn_topology_t` | structural reshape (#33): `contour_count` (uint32_t) promoted from implicit 4-byte padding at offset 84; `contours` (const gn_topo_contour_t\*) new named field at offset 88; `_reserved[4]` shifts to offset 96; sizeof grows 120 → 128 bytes. New `gn_topo_contour_t` struct added in `sdk/topology.h` — named packet path carrying `trust`, `link_scheme`, `security_provider_id`, `security_provides_flags`, `protocol_id`, `handler_count`, `handler_msg_ids`. ABI-safe: `_reserved[4]` count unchanged at 4; all pointer fields `@borrowed` from kernel storage. See `layer-capability.en.md §3`. | `feat/named-contours-33` |
