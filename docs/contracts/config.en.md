...
_16 config key(s) read by core / plugins, grouped by family._

### `ice` keys

| Key | Type | Reader |
|---|---|---|
| `ice.aggressive_nomination` | `int64` | [`plugins/links/ice/link_ice.cpp#L239`](../../plugins/links/ice/link_ice.cpp#L239) |
| `ice.check_interval_ms` | `int64` | [`plugins/links/ice/link_ice.cpp#L218`](../../plugins/links/ice/link_ice.cpp#L218) |
| `ice.consent_max_failures` | `int64` | [`plugins/links/ice/link_ice.cpp#L214`](../../plugins/links/ice/link_ice.cpp#L214) |
| `ice.keepalive_interval_s` | `int64` | [`plugins/links/ice/link_ice.cpp#L210`](../../plugins/links/ice/link_ice.cpp#L210) |
| `ice.path_mtu` | `int64` | [`plugins/links/ice/link_ice.cpp#L230`](../../plugins/links/ice/link_ice.cpp#L230) |
| `ice.session_timeout_s` | `int64` | [`plugins/links/ice/link_ice.cpp#L206`](../../plugins/links/ice/link_ice.cpp#L206) |
| `ice.turn_password` | `string` | [`plugins/links/ice/link_ice.cpp#L195`](../../plugins/links/ice/link_ice.cpp#L195) |
| `ice.turn_tcp` | `int64` | [`plugins/links/ice/link_ice.cpp#L246`](../../plugins/links/ice/link_ice.cpp#L246) |
| `ice.turn_tls` | `int64` | [`plugins/links/ice/link_ice.cpp#L252`](../../plugins/links/ice/link_ice.cpp#L252) |
| `ice.turn_username` | `string` | [`plugins/links/ice/link_ice.cpp#L182`](../../plugins/links/ice/link_ice.cpp#L182) |
...
### 3b. Per-section reload

`Config::merge_section(prefix, section_json)` reloads only the subtree under
`prefix` (a dotted-path top-level key or path such as `"ice"`, `"links.tls"`)
without touching any other key in the live document. It is semantically
equivalent to `merge_json({"<prefix>": parse(section_json)})` and carries
the same atomicity guarantee: a parse or invariant failure rolls back to the
prior state.

The operator-side entry point is:
...
On success the kernel fires `on_config_reload` exactly once, so every
subscriber (global or section-scoped) sees the change.

Plugin-side subscriptions can narrow to a namespace through:

```c
host_api->subscribe_config_reload_section(host_ctx, "ice",
                                           my_reload_cb, …, &sub_id);
```

**v1 semantics:** the kernel fires the callback on every `on_config_reload`
regardless of which subtree changed. Section-aware dispatch (firing only when
the subscriber's prefix appears in the changed subtree) is a v2 enhancement —
the callback signature is already right for it.

**Intended use:** each plugin owns a named namespace in the config tree and
subscribes to its own prefix. The operator calls
`gn_core_reload_config_section(core, "ice", new_ice_json)` to push updated
TURN credentials without disturbing TLS or heartbeat knobs, and only
ICE-namespace subscribers receive the callback burst. This pattern eliminates
the "global reload to change one plugin's config" footgun and matches
`docs/contracts/config.en.md` §2 (plugin isolation).
...
- **Per-plugin schema discovery.** Plugins do not register the
  ...
  its built-in default — the operator gets no warning. Per-section
  ownership (§3b) narrows _which_ subtree a plugin reads, but does
  not yet validate key names within that subtree. A `reads_config`
  whitelist in `plugin-manifest.en.md` is planned so the kernel
  logs unknown-key warnings at load time.
