...
`host_api_t` exposes **43 named slots** plus `8` reserved pointers for forward-compatible growth, declared in [`sdk/host_api.h`](../../sdk/host_api.h).
...
- **Per-section config-reload subscription** — `subscribe_config_reload_section`
...
- **Topology-reload subscription** — `subscribe_topology_reload`
...
| [subscribe_topology_reload](../../sdk/host_api.h#L787) | `gn_result_t (*)(void *, gn_topology_reload_cb_t, void *, void (*)(void *), gn_subscription_id_t *)` | Topology-reload subscription |
| [subscribe_config_reload_section](../../sdk/host_api.h#L808) | `gn_result_t (*)(void *, const char *, gn_config_reload_cb_t, void *, void (*)(void *), gn_subscription_id_t *)` | Per-section config-reload subscription |
<!-- /livedoc:host_api_slots -->
...
**`reads_config` prefix gate.** If the plugin's descriptor declares a non-empty
`reads_config` list, the kernel enforces it at call time: any key that does not
start with one of the declared prefixes is rejected with `GN_ERR_NOT_FOUND`
(indistinguishable from a legitimately absent key). An empty or absent
`reads_config` (NULL in the C descriptor) imposes no restriction.
...
| key not present in config, **or** key blocked by `reads_config` prefix gate | `GN_ERR_NOT_FOUND` |
...
| caller's descriptor has non-empty `inject_targets` and `(target_ns, msg_id)` is not declared (`msg_id==0` in declaration = wildcard) | `GN_ERR_INVALID_ENVELOPE` |
