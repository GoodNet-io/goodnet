...
| `provides_flags` | bitmask of `GN_SEC_PROVIDES_*` constants; read once at topology-build time (see `layer-capability.en.md §7`); NULL-safe — old plugins that omit the slot return 0 via `GN_API_HAS` guard |
...
- The active **security provider** declares its admitted classes via
  ...
  `GN_ERR_INVALID_ENVELOPE` and increments the same metric. The
  provider also declares its cryptographic capability tier through
  `provides_flags` (`GN_SEC_PROVIDES_E2E_ENCRYPTION`,
  `GN_SEC_PROVIDES_AUTHENTICATION`, `GN_SEC_PROVIDES_FORWARD_SECRECY`);
  these flags are read once at topology-build time and stored in
  `gn_topo_security_entry_t::provides_flags` — see
  `layer-capability.en.md §7`.
...
- Capability topology snapshot, `provides_flags` flag values, contour proof:
  `layer-capability.en.md`.
