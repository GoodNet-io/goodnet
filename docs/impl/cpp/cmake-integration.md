...
- Minor — ниже, чем у плагина — `GN_ERR_VERSION_MISMATCH` на
  `query_extension_checked`, см. [abi-evolution](../../contracts/abi-evolution.md).
...
артефакт, см. [plugin-manifest](../../contracts/plugin-manifest.md).
...
- [plugin-manifest contract](../../contracts/plugin-manifest.md) —
  что попадает в `<plugin>.manifest.json`.
- [abi-evolution contract](../../contracts/abi-evolution.md) —
  правила major/minor SDK-версионирования.
- [overview](./overview.md) — общий контекст C++ обёрток.
- [write-handler-plugin recipe](../../recipes/write-handler-plugin.md) —
