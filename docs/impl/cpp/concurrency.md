...
Контракт [`link.md §9`](../../contracts/link.md) говорит:
...
Оба случая нарушают
[`link.md §9`](../../contracts/link.md): для опубликованной через
...
- [`link.md` §9](../../contracts/link.md) — формальный контракт
  shutdown release
- [`signal-channel.md`](../../contracts/signal-channel.md) — события,
  которые ядро фаерит в ответ на `notify_disconnect`
- [`memory-management.md`](./memory-management.md) — borrowed/owned
  семантика на C ABI границе
- [`error-handling.md`](./error-handling.md) — `gn_result_t`
  семантика для `(void)`-cast'а ошибок ожидаемого типа
- [`plugin-lifetime.md`](../../contracts/plugin-lifetime.md) §4 —
