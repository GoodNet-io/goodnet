...
`gn_timer_id_t`. См. [memory-management](memory-management.ru.md) §subscription-lifetime.
...
1. **`noexcept` + `try/catch` всегда.** Исключение не должно
   ...
   код. См. [error-handling](error-handling.ru.md) §no-exceptions.
...
- [error-handling](error-handling.ru.md) — gn_result_t,
  std::expected, перенос ошибок через C ABI.
- [memory-management](memory-management.ru.md) — owned/borrowed,
...
- [plugin-model](../../architecture/plugin-model.ru.md) — какие kind'ы
  плагинов существуют, как ядро их грузит.
- [write-handler-plugin](../../recipes/write-handler-plugin.ru.md) —
  пошаговый рецепт первого handler-плагина.
- [host-api](../../contracts/host-api.en.md) — полная спецификация
