...
big-endian per [identity.md](../contracts/identity.md) §4.
...
[noise-handshake](./noise-handshake.md) и
...
| Attacker украл оба | leaked-key attack, см. [attestation.md](../contracts/attestation.md) §10 |
...
4. **Cert parse.** 136-байт cert не парсится per
   [identity.md](../contracts/identity.md) §4 →
...
7. **Cross-session identity stability.** `ConnectionRegistry::
   ...
   [registry.md](../contracts/registry.md) §8a).
...
3. Connection close через `notify_disconnect(conn, reason)`
   ([conn-events.md](../contracts/conn-events.md) §2a).
...
device replacement scenario. Address (per [identity.md](../contracts/identity.md) §3) меняется,
...
- [attestation.md](../contracts/attestation.md) — kernel-side dispatcher
  spec, mutual-exchange flow, leaked-key model
- [identity.md](../contracts/identity.md) — `KeyPair`, `NodeIdentity`,
  cert layout source
...
- [noise-handshake](./noise-handshake.md) — где `handshake_hash`
  binding'а формируется
- [registry.md](../contracts/registry.md) §8a — cross-session pin
  semantics
