...
- `length > kMaxFrameBytes` (default 65536) → `GN_ERR_FRAME_TOO_LARGE`
  и counter `drop.frame_too_large` ([metrics.md](../contracts/metrics.md)
  §3)
...
([protocol-layer.md](../contracts/protocol-layer.md) §5).
...
| `0x11` | attestation dispatcher | 232 байта signed payload, см. [attestation-bytes](./attestation-bytes.md) |
| `0x12` | capability TLV blob | TLV records, см. [capability-tlv.md](../contracts/capability-tlv.md) |

Регистрация handler'а на любой kernel-canon `msg_id` (через
`register_vtable(GN_REGISTER_HANDLER)`) отклоняется с
`GN_ERR_INVALID_ENVELOPE`
([handler-registration.md](../contracts/handler-registration.md) §2a).
...
([protocol-layer.md](../contracts/protocol-layer.md) §2.2).
...
Cross-link: [backpressure.md](../contracts/backpressure.md) формализует
...
- [protocol-layer.md](../contracts/protocol-layer.md) — kernel-side
  envelope semantics (`gn_message_t` lifetime, identity sourcing)
- [routing](../architecture/routing.md) — что kernel делает между
  `deframe` и handler dispatch
- [wire-protocol](../architecture/wire-protocol.md) — почему
  mesh-framing мандаторен
- [limits.md](../contracts/limits.md) §2 — `max_frame_bytes`,
  `max_payload_bytes`
- [handler-registration.md](../contracts/handler-registration.md) §2a —
  reserved msg_id ranges, kernel-canon allocation
- [capability-tlv.md](../contracts/capability-tlv.md) — TLV-blob,
  едущий по `msg_id == 0x12`
- [noise-handshake](./noise-handshake.md) — security-слой, который
  обнимает GNET frame с обеих сторон
- [attestation-bytes](./attestation-bytes.md) — payload `msg_id == 0x11`
