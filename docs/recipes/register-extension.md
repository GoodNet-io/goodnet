...
- [host-api.md §2 (Extension API)](../contracts/host-api.md) — slot definitions для `register_extension` / `query_extension_checked` / `unregister_extension`.
- [extension-model](../architecture/extension-model.md) — почему extensions, namespace conventions, anti-patterns.
- [relay-direct](../architecture/relay-direct.md) — пример consumer-side use case.
- [plugin-lifetime.md §4](../contracts/plugin-lifetime.md) — auto-reap registrations через lifetime-anchor.
- [abi-evolution.md §3](../contracts/abi-evolution.md) — size-prefix evolution для vtable shape.
