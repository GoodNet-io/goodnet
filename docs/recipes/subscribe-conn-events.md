...
- [conn-events.md](../contracts/conn-events.md) — event kinds, DISCONNECTED spec, ordering, dropped events.
- [signal-channel.md](../contracts/signal-channel.md) — pub/sub primitive под капотом, snapshot rule, exception-catch contract.
- [host-api.md §2](../contracts/host-api.md) — `subscribe_conn_state` / `unsubscribe` / `for_each_connection` slot signatures.
- [relay-direct](../architecture/relay-direct.md) — пример consumer'а для TRUST_UPGRADED.
- [timer.md §2-3](../contracts/timer.md) — куда постить долгую работу из callback'а.
- [plugin-lifetime.md §4](../contracts/plugin-lifetime.md) — auto-reap subscriptions через lifetime anchor.
