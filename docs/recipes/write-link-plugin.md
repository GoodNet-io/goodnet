...
Парсинг URI делает ядро через [uri.md](../contracts/uri.md). Authority-
...
[security-trust.md](../contracts/security-trust.md) §3). AF_UNIX —
...
- [link.md](../contracts/link.md) — каноничный link-контракт: ABI,
  TrustClass declaration, single-writer invariant, shutdown release.
- [uri.md](../contracts/uri.md) — формат URI, kernel-side парсер,
  hostname resolution rules.
- [host-api.md](../contracts/host-api.md) — `notify_connect`,
...
- [backpressure.md](../contracts/backpressure.md) — soft/hard
...
- [routing](../architecture/routing.md) — куда ядро отправляет байты
  после `notify_inbound_bytes`.
- [plugin-model](../architecture/plugin-model.md) — общая
