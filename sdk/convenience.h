 ...
 * into `out_str`, an opaque destruction-state handle into
 * `out_user_data`, and the matching destructor into `out_free`;
 * the plugin frees through (*out_free)(*out_user_data, *out_str). */
#define gn_config_get_string(api, key, out_str, out_user_data, out_free)       \
    (api)->config_get((api)->host_ctx, (key),                                  \
                      GN_CONFIG_VALUE_STRING, GN_CONFIG_NO_INDEX,              \
                      (void*)(out_str), (out_user_data), (out_free))

#define gn_config_get_int64(api, key, out_value)                               \
    (api)->config_get((api)->host_ctx, (key),                                  \
                      GN_CONFIG_VALUE_INT64, GN_CONFIG_NO_INDEX,               \
                      (void*)(out_value), NULL, NULL)
...
#define gn_config_get_array_string(api, key, index, out_str, out_user_data, out_free) \
    (api)->config_get((api)->host_ctx, (key),                                  \
