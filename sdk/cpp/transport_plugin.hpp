/// @file   sdk/cpp/transport_plugin.hpp
/// @brief  `TRANSPORT_PLUGIN(Class, scheme)` — collapses every
///         transport plugin's `plugin_entry.cpp` boilerplate into one
///         macro instantiation.
///
/// The boilerplate it replaces:
///
/// 1. Five `gn_plugin_*` extern "C" entry points.
/// 2. The `gn_transport_vtable_t` thunks bridging the C ABI to the
///    transport's C++ method shapes.
/// 3. The `gn.transport.<scheme>` extension vtable plus its thunks
///    (steady slots functional, composer slots returning
///    `GN_ERR_NOT_IMPLEMENTED` per `transport.md` §8 staged delivery).
...
/// static gn_transport_caps_t capabilities() noexcept;
/// ```
///
...
/// and never re-reads it (per `transport.md` §8 capabilities are
...
/// `TRANSPORT_PLUGIN(Class, "scheme")`. See file header for the class
...
#define TRANSPORT_PLUGIN(Class, SchemeStrLiteral)                              \
    namespace {                                                                \
    using _gn_tp_instance_t = ::gn::sdk::detail::TransportPluginInstance<Class>;\
    constexpr const char  _gn_tp_scheme[]   = SchemeStrLiteral;                \
    constexpr const char  _gn_tp_extension_prefix[] = GN_EXT_TRANSPORT_PREFIX; \
    constexpr const char  _gn_tp_plugin_name[] = "goodnet_transport_" SchemeStrLiteral; \
                                                                               \
    inline auto& _gn_tp_of(void* self) {                                       \
        return *static_cast<_gn_tp_instance_t*>(self)->transport;              \
    }                                                                          \
                                                                               \
    /* ── kernel-facing transport vtable ──────────────────────── */          \
    ...
    gn_result_t _gn_tp_ext_get_caps(                                           \
        void* ctx, gn_transport_caps_t* out) noexcept {                        \
        if (!ctx || !out) return GN_ERR_NULL_ARG;                              \
    ...
    gn_result_t _gn_tp_ext_send(                                               \
        ...
        try {                                                                  \
            auto* inst = static_cast<_gn_tp_instance_t*>(ctx);                 \
            return inst->transport->send(conn,                                 \
                std::span<const std::uint8_t>(bytes, size));                   \
        } catch (...) { return GN_ERR_NULL_ARG; }                              \
    }                                                                          \
    gn_result_t _gn_tp_ext_send_batch(                                         \
        ...
        try {                                                                  \
            ...
            return ::gn::sdk::detail::batch_through(                           \
                *inst->transport, conn, batch, count);                         \
        } catch (...) { return GN_ERR_NULL_ARG; }                              \
    }                                                                          \
    gn_result_t _gn_tp_ext_close(                                              \
        ...
        try {                                                                  \
            auto* inst = static_cast<_gn_tp_instance_t*>(ctx);                 \
            return inst->transport->disconnect(conn);                          \
        } catch (...) { return GN_ERR_NULL_ARG; }                              \
    }                                                                          \
    ...
    gn_result_t _gn_tp_ext_subscribe_unimpl(                                   \
        void*, gn_conn_id_t,                                                   \
        gn_transport_data_callback_t, void*) noexcept {                        \
        return GN_ERR_NOT_IMPLEMENTED; }                                       \
    ...
    void _gn_tp_install_ext(_gn_tp_instance_t* inst) noexcept {                \
        ...
        v               = gn_transport_api_t{};                                \
        v.api_size      = sizeof(gn_transport_api_t);                          \
        ...
        std::memcpy(inst->extension_name_buf + sizeof(_gn_tp_extension_prefix) - 1, \
                    ...
                    sizeof(_gn_tp_scheme));                                    \
                                                                               ...
                                                                               \
    gn_transport_vtable_t _gn_tp_make_vtable() noexcept {                      \
        ...
        return v;                                                              \
                                                                               ...
                                                                               \
    const gn_transport_vtable_t _gn_tp_kVtable = _gn_tp_make_vtable();         \
                                                                               \
    const char* const _gn_tp_kProvides[] = {                                   \
        "gn.transport." SchemeStrLiteral,                                      \
        nullptr,                                                               \
    };                                                                         \
                                                                               \
    ...
    GN_PLUGIN_EXPORT gn_result_t gn_plugin_init(                               \
        ...
        try {                                                                  \
            ...
            p->transport = std::make_shared<Class>();                          \
    ...
    GN_PLUGIN_EXPORT gn_result_t gn_plugin_register(void* self) {              \
        ...
        if (auto rc = p->api->register_transport(                              \
    ...
    GN_PLUGIN_EXPORT gn_result_t gn_plugin_unregister(void* self) {            \
        ...
        if (p->api && p->api->unregister_transport &&                          \
            ...
            (void)p->api->unregister_transport(                                \
                p->host_ctx, p->transport_id);                                 \
            p->transport_id = GN_INVALID_ID;                                   \
        }                                                                      \
