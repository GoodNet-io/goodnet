/**
 * @file   sdk/extensions/transport.h
 * @brief  Per-transport extension surface: `gn.transport.<scheme>`.
 *
 * Transports register a typed extension vtable under
 * `gn.transport.<scheme>` (e.g. `gn.transport.tcp`,
 * `gn.transport.udp`, `gn.transport.ipc`) so other plugins can
 *
 *   1. snapshot per-transport counters (`get_stats`),
 *   2. read static capabilities (`get_capabilities`) without parsing
 *      strings or hard-coding scheme assumptions, and
 *   3. compose layer-2 transports on top of layer-1 ones —
 ...
 * transport target an arbitrary L1 without scheme-aware glue.
 *
 * See `docs/contracts/transport.md` §8 for the registration model and
 * the per-transport `<scheme>.h` headers for capability flag values
 ...
 *        `"gn.transport." + scheme`, lowercase.
 */
#define GN_EXT_TRANSPORT_PREFIX "gn.transport."

/** v1.0.0 — initial release. */
#define GN_EXT_TRANSPORT_VERSION 0x00010000u

/**
 * @brief Capability flags, ORed into `gn_transport_caps_t::flags`.
 *
 * `Stream` means the transport delivers an unframed byte stream and
 ...
 *
 * `EncryptedPath` is the transport asserting it carries already-
 * encrypted bytes (e.g. a TLS terminator). `LocalOnly` is the
 * transport refusing to bind a public address regardless of URI
 * (loopback, AF_UNIX).
 */
#define GN_TRANSPORT_CAP_STREAM         (1u << 0)
#define GN_TRANSPORT_CAP_DATAGRAM       (1u << 1)
#define GN_TRANSPORT_CAP_RELIABLE       (1u << 2)
#define GN_TRANSPORT_CAP_ORDERED        (1u << 3)
#define GN_TRANSPORT_CAP_ENCRYPTED_PATH (1u << 4)
#define GN_TRANSPORT_CAP_LOCAL_ONLY     (1u << 5)

/**
 * @brief Static capability descriptor. Stable for the lifetime of the
 *        transport plugin; the kernel may snapshot once at register time.
 */
typedef struct gn_transport_caps_s {
    uint32_t flags;          /**< OR of `GN_TRANSPORT_CAP_*` */
 ...
 *        transport's lifetime; rollover is handled by the consumer.
 */
typedef struct gn_transport_stats_s {
    uint64_t bytes_in;
    uint64_t bytes_out;
    uint64_t frames_in;
    uint64_t frames_out;
...
} gn_transport_stats_t;

/**
 * @brief Receive-side callback for composer plugins.
 *
 * Called by the L1 transport when bytes arrive on a connection a
 * composer subscribed to via @ref gn_transport_api_t::subscribe_data.
 * `bytes` is `@borrowed` for the duration of the call; the composer
 * must copy if it needs to retain past return.
 */
typedef void (*gn_transport_data_callback_t)(
    void*          user_data,
    gn_conn_id_t   conn,
    const uint8_t* bytes,
    size_t         size);
...
/**
 * @brief Vtable surfaced under `gn.transport.<scheme>`.
 *
 ...
 *     `send_batch`, `close`. Implemented by every baseline transport
 ...
 *     kernel's `notify_connect` flow. Baseline transports return
 *     `GN_ERR_NOT_IMPLEMENTED` until the corresponding L2 transport
 ...
 */
typedef struct gn_transport_api_s {
    uint32_t api_size;          /**< sizeof(gn_transport_api_t) at producer build time */

    /* ── Steady slots ───────────────────────────────────────────── */

    /**
     * @brief Snapshot per-transport counters into @p out.
     *
     * @return @ref GN_OK on success, @ref GN_ERR_NULL_ARG when
     *         @p ctx or @p out is NULL.
     */
    gn_result_t (*get_stats)(void* ctx, gn_transport_stats_t* out);

    /**
     * @brief Read static capability flags into @p out. Values stable
     *        for the plugin's lifetime; safe to cache.
     */
    gn_result_t (*get_capabilities)(void* ctx, gn_transport_caps_t* out);

    /**
     * @brief Send bytes on @p conn. Same shape as the kernel-facing
     *        `gn_transport_vtable_t::send`; intended for composer
     *        plugins that have a kernel `gn_conn_id_t` in hand.
     *
     * Single-writer invariant per `transport.md` §4 applies.
     */
     ...
     *
     * Returns @ref GN_ERR_NOT_IMPLEMENTED on baseline transports in
     * v1.0.x — see contract `transport.md` §8.
     */
    ...
    gn_result_t (*subscribe_data)(void* ctx, gn_conn_id_t conn,
                                  gn_transport_data_callback_t cb,
                                  void* user_data);
