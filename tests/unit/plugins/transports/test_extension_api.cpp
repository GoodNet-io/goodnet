/// @file   tests/unit/plugins/transports/test_extension_api.cpp
/// @brief  `gn.transport.<scheme>` extension API surface — every
...
#include <plugins/transports/ipc/ipc.hpp>
...
struct TransportInstance {
    ...
    explicit TransportInstance(ExtensionFixture& f, const std::string& scheme) {
        ...
        caps      = T::capabilities();
        ext_name  = std::string{"gn.transport."} + scheme;
        ...
        ext_vtable               = gn_transport_api_t{};
        ext_vtable.api_size      = sizeof(gn_transport_api_t);
    ...
    static gn_result_t ext_get_stats(void* c, gn_transport_stats_t* out) {
    ...
    static gn_result_t ext_get_caps(void* c, gn_transport_caps_t* out) {
        if (!c || !out) return GN_ERR_NULL_ARG;
...
template <class T>
const gn_transport_api_t* lookup_extension(ExtensionFixture& f,
    ...
    return static_cast<const gn_transport_api_t*>(raw);
}
...
TEST(TransportExtensionApi, TcpExposesStreamCapabilities) {
    ExtensionFixture f;
    TransportInstance<gn::transport::tcp::TcpTransport> tcp(f, "tcp");

    auto* api = lookup_extension<gn::transport::tcp::TcpTransport>(
        f, "gn.transport.tcp");
...
TEST(TransportExtensionApi, IpcAdvertisesLocalOnly) {
    ExtensionFixture f;
    TransportInstance<gn::transport::ipc::IpcTransport> ipc(f, "ipc");

    auto* api = lookup_extension<gn::transport::ipc::IpcTransport>(
        f, "gn.transport.ipc");
    ASSERT_NE(api, nullptr);

    gn_transport_caps_t caps{};
    ASSERT_EQ(api->get_capabilities(api->ctx, &caps), GN_OK);
    EXPECT_TRUE(caps.flags & GN_TRANSPORT_CAP_LOCAL_ONLY);
    EXPECT_TRUE(caps.flags & GN_TRANSPORT_CAP_STREAM);
}

TEST(TransportExtensionApi, UdpAdvertisesDatagram) {
    ExtensionFixture f;
    TransportInstance<gn::transport::udp::UdpTransport> udp(f, "udp");

    auto* api = lookup_extension<gn::transport::udp::UdpTransport>(
        f, "gn.transport.udp");
    ASSERT_NE(api, nullptr);

    gn_transport_caps_t caps{};
    ASSERT_EQ(api->get_capabilities(api->ctx, &caps), GN_OK);
    EXPECT_TRUE(caps.flags & GN_TRANSPORT_CAP_DATAGRAM);
    EXPECT_FALSE(caps.flags & GN_TRANSPORT_CAP_STREAM);
