...
/// @file   tests/unit/plugins/transports/test_ipc.cpp
/// @brief  IpcTransport — listen+connect on a per-PID socket path,
...
TEST(IpcTransport, ListenCreatesSocketFile) {
    const auto path = make_socket_path();
    const auto uri  = "ipc://" + path;

    auto t = std::make_shared<IpcTransport>();
    StubHost h;
...
TEST(IpcTransport, LoopbackHandshakeAndPayloadRoundTrip) {
    const auto path = make_socket_path();
    const auto uri  = "ipc://" + path;
    ...
    /// Both sides report Loopback per `transport.md` §3.
    {
        std::lock_guard lk(h.mu);
        ASSERT_EQ(h.trusts.size(), 2u);
        for (auto t : h.trusts) EXPECT_EQ(t, GN_TRUST_LOOPBACK);
    }
...
TEST(IpcTransport, ConnectRejectsNonExistentSocket) {
    StubHost h;
    auto api = make_stub_api(h);
    auto t = std::make_shared<IpcTransport>();
    t->set_host_api(&api);
