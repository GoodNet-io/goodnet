...
/// @file   tests/unit/plugins/transports/test_tcp.cpp
/// @brief  TcpTransport — listen+connect loopback drives the full
...
TEST(TcpTransport, ListenOnEphemeralPortReturnsNonZero) {
    StubHost h;
...
TEST(TcpTransport, ListenRejectsMalformedUri) {
    auto t = std::make_shared<TcpTransport>();
...
TEST(TcpTransport, LoopbackHandshakeAndPayloadRoundTrip) {
    StubHost h;
    auto api = make_stub_api(h);
    auto t = std::make_shared<TcpTransport>();
    t->set_host_api(&api);
...
TEST(TcpTransport, SendBatchCoalescesIntoOneStream) {
    StubHost h;
    auto api = make_stub_api(h);
    auto t = std::make_shared<TcpTransport>();
    t->set_host_api(&api);
